...
def branch_r_l(k: int, j: int, eg: np.array, au: np.array, pg: np.array, pkj: float, pk: float, _r_l_=None):
    """
    本函数用于计算支路碳流率
...
def bus_r_l(k: int, eg: np.array, au: np.array, pg: np.array, plk: float, pk: float, _r_l_=None):
    """
    本函数用于计算节点碳流率
    k: form节点编号，从0开始，即0对应节点编号1
    j: to节点编号
...
def loss_r_l(k: int, j: int, eg: np.array, au: np.array, pg: np.array, pkj_loss: float, pk: float, _r_l_=None):
    """
    本函数用于计算网损碳流率
    k: form节点编号，从0开始，即0对应节点编号1
    j: to节点编号
...
def r_l_2g4b(eg: np.array, au: np.array, pg: np.array,
    ...
    for k in range(bus_num):
        _r_l_ = _r_l(k, eg, au, pg)[0, 0]
        bus_res[k] = bus_r_l(k, eg, au, pg, plk = plk_lis[k], pk = pk_lis[k], _r_l_= _r_l_)
        for key, value in pkj_loss_dict.items():
            if key[0] == str(k+1):
                branch_res[key] = float(branch_r_l(k, eval(key[-1]), eg, au, pg, pkj=value[0], pk=pk_lis[k], _r_l_= _r_l_))
                loss_res[key] = float(loss_r_l(k, eval(key[-1]), eg, au, pg, pkj_loss=value[1], pk=pk_lis[k], _r_l_= _r_l_))
